...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", type=Path, required=True, help="Source directory containing images")
    parser.add_argument("-o", "--output", type=str, required=True,
                        help="Output filename template, e.g. 'folder/frame_{frame}.png'")
    parser.add_argument("--decay", type=float, default=0.99, help="Brightness decay factor (default: 0.99)")
    parser.add_argument("--agg", type=str, choices=["maximum", "minimum"], default="maximum",
                        help="Aggregation function (default: maximum)")
    args = parser.parse_args()

    agg_fun_map = {"maximum": np.maximum, "minimum": np.minimum}
    agg_fun = agg_fun_map[args.agg]

    import cv2

    image_files = sorted([f for f in args.input.iterdir() if f.suffix.lower() in (".jpg", ".jpeg", ".png")])
    ...
    for i, frame in enumerate(star_trails_video(src=images, decay=args.decay, agg_fun=agg_fun)):
        output_path = Path(args.output.format(frame=i))
        output_path.parent.mkdir(parents=True, exist_ok=True)
        cv2.imwrite(str(output_path), frame)
    print(f"Saved frames to: {Path(args.output).parent}")
